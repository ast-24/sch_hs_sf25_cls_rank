...
import { updateUserScoreCache } from "./user_score_cache.mjs";
...
export async function updateUserResults(tidbCl, userDbId, newResults) {
    ...
    for (const [newRoundId, newRoundData] of Object.entries(newResults)) {
        if (!oldResults[newRoundId]) {
            throw new Error(`Round ID ${newRoundId} not found for user ${userDbId}`);
        }
    ...
    await updateUserScoreCache(tidbCl, userDbId, Object.keys(newResults));
}